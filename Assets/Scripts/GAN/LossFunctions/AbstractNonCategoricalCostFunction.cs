...
namespace SharpGAN.CostFunctions
{
    ...
    public abstract class AbstractNonCategoricalCostFunction : AbstractCostFunction
    {
        ...
        public override void TestDimension(double[][][][] input, double[][] targets, int batchStart = 0, int batchEnd = 0)
        {
            ...
            {
                UnityEngine.Debug.Log("Invalid input data in mean squared error, expected input (" + numSamples + "," + numClasses + ") and found (" + exNumSamples + "," + exNumClasses + ")");
            }

            if (numClasses != 1 || exNumClasses != 1)
            {
                UnityEngine.Debug.Log("Invalid input data in mean squared error, expected input with non categorical data. Which has " +
                    "one output.");
