...
namespace SharpGAN.Loaders
{
    ...
    /// </summary>
    [Serializable]
    public abstract class AbstractLoader1D : AbstractLoader
    {
        /// <summary>
        ...
        public AbstractLoader1D(int trainItemCount, int testItemCount, int batchSize)
        {
        ...
        public override Tuple<double[][][][], double[][]> LoadBatch(int batchIndex, bool train)
        {
            ...
            for (int i = from; i < to; i++)
            {
                ...
                batchValues[counter] = ConvertTo3D(item.Item1);
        ...
        private double[][][] ConvertTo3D(double[] flatten)
        {
            double[][][] array3d = new double[1][][];
            array3d[0] = new double[1][];
            array3d[0][0] = flatten;
