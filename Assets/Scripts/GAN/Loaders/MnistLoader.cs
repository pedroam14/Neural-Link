...
namespace SharpGAN.Loaders
{
    /// <summary>
    /// Loader which will load the MNIST dataset from disk
    /// http://yann.lecun.com/exdb/mnist/
    /// </summary>
    [Serializable]
    public class MnistLoader : AbstractLoader2D
    {
        /// <summary>
        ...
        public MnistLoader(int trainItemCount, int testItemCount, int batchSize, bool categorical = true) 
        ...
        private Tuple<double[][][], double[]> ReadLabelAndImage(string imagesAbsPath, string labelsAbsPath, int index, bool train)
        {
            ...
            try
            {
                ...
                readerLabels.ReadInt32();
                // init image array
                image = new double[DEPTH_OF_IMAGE][][];
                ...
                double[][] pixels = new double[HEIGHT_PX][];
                // init pixels array
                for (int i = 0; i < pixels.Length; ++i)
                    pixels[i] = new double[WIDTH_PX];
                ...
                if (categorical)
                {
                    Tuple<double[][], Dictionary<int, uint>> res = Common.Utils.ToCategorical(labelsInt, CLASSES_COUNT, classes);
                    labels = res.Item1[0];
                    classes = res.Item2;
                }
                else
