...
namespace SharpGAN.Model
{
    ...
    public class SequentialModel : AbstractModel
    {
        ...
        public void DBackwardPropagation(double[][][][] actual, double[][][][] expected)
        {

            var currentGradient = lossFunction.Derivate(predicts: actual, targets: expected[0][0]);
            ...
            for (int i = 0; i < layers.Count; i++)
            {
                ...
                {
                    optimizer.UpdateWeights((ILearnable)layers[i]);
                }
            }
        }
        public void GBackwardPropagation(double[][][][] actual, double[][][][] expected)
        {
            var currentGradient = lossFunction.Derivate(predicts: actual, targets: expected);
            for (int i = layers.Count - 1; i >= 0; i--)
            {
                currentGradient = layers[i].BackwardPropagation(currentGradient);
            }
