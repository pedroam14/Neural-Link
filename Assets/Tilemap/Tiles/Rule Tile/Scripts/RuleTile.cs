...
namespace UnityEngine
{
    ...
    public class RuleTile : TileBase
    {
        ...
        public class TilingRule
        {
            ...
            public enum OutputSprite { Single, Random, Animation, RandomAnimation }
        }
                ...
                if (RuleMatches(rule, ref neighboringTiles, ref transform))
                {
                    switch (rule.m_Output)
                    {
                        ...
                        case TilingRule.OutputSprite.Random:
                            ...
                            break;
                        case TilingRule.OutputSprite.RandomAnimation:
                            tileData.sprite = rule.m_Sprites[Random.Range(0, rule.m_Sprites.Length - 1)];
                            break;
        ...
        public override bool GetTileAnimationData(Vector3Int position, ITilemap tilemap, ref TileAnimationData tileAnimationData)
        {
            ...
            foreach (TilingRule rule in m_TilingRules)
            {
                if (rule.m_Output == TilingRule.OutputSprite.Animation)
                {
                    ...
                    if (RuleMatches(rule, ref neighboringTiles, ref transform))
                    {
                        tileAnimationData.animatedSprites = rule.m_Sprites.OrderBy(x => rnd.Next()).ToArray();
                        tileAnimationData.animationSpeed = Random.Range(1,3)*rule.m_AnimationSpeed;
                        return true;
                    }
                }
            }
